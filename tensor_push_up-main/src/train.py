...
class Trainer:
    ...
    def build_model(self, resume_from: str = None):
        ...
        window_size = self.data_config.get('window_size', 30)
        num_keypoints = self.data_config.get('num_keypoints', 33)
        num_angles = self.data_config.get('num_angles', 8)

        # Calculate feature dimensions
        # Using the same feature extraction as in pose_estimator
        # Selected keypoints: 22 points * 2 coords = 44 + 8 angles = 52
        feature_dim = 50  # Matching pose_estimator.extract_features output
    ...
    def evaluate(self):
        ...
        for i, row in enumerate(cm):
            logger.info(f"  {ACTION_CLASSES.get(i, 'unknown')}: {row}")
    ...
    def run(self, resume_from: str = None, export: bool = True):
        """
...
def main():
    ...
    # Run training
    trainer.run(resume_from=args.resume, export=not args.no_export)
